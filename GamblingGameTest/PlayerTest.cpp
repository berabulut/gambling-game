...
TEST(PlayerTest, GetName) {
	...
	Player* p = new Player(name, 1000, 0.1, 1);
...
TEST(PlayerTest, GetCash) {
	...
	Player* p = new Player("name", startingCash, 0.1, 1);
...
TEST(PlayerTest, CalculateRoundBet) {
	...
	Player* p = new Player("name", startingCash, betRate, 1);
...
TEST(PlayerTest, HasPlayerWonRound) {
	Player* p1 = new Player("name", 1000, 0.1, 9);
	EXPECT_EQ(true, p1->hasPlayerWonRound(9));
	delete p1;

	Player* p2 = new Player("name", 1000, 0.1, 7);
	EXPECT_EQ(true, p2->hasPlayerWonRound(7));
	delete p2;

	Player* p3 = new Player("name", 1000, 0.1, 5);
	EXPECT_EQ(false, p3->hasPlayerWonRound(7));
	delete p3;

	Player* p4 = new Player("name", 1000, 0.1, 2);
...
TEST(PlayerTest, SetNewCashWonRound) {
	...
	Player* p = new Player("name", startingCash, betRate, 1);
	p->setNewCash(true);
	EXPECT_EQ(int(323845.5), int(p->getCash()));
	delete p;
}
